...
class HexChessEnv(gymnasium.Env):
    ...
    def __init__(self, max_steps: int = 128, render_mode: str = "human") -> None:
        self.space_size = 3 * 5 + 6 + 22 * 2 + 22 * 2 + 6 * 2 + 12 * 2
        self.action_space = spaces.Discrete(self.space_size)
        self.grid_height = 7
        self.grid_width = 10
        self.hex_size = 1
        self.observation_space = spaces.Box(low=0, high=2,
                  shape=(self.grid_height, self.grid_width, 1), dtype=np.int8)
        ...
        self.turn: int = 0  # 0 for white, 1 for black
    ...
    def is_valid_move(self, next_pos, turn):
      """Check if the move from pos to next_pos is valid based on the board constraints and turn."""
      next_row, next_col = next_pos

      # Проверка, находится ли следующая позиция в пределах доски и не включена ли она в список исключенных позиций
      excluded_positions = [(6, 1), (6, 3), (6, 5), (6, 7)]
      if not (0 <= next_row < self.grid_height and 0 <= next_col < self.grid_width - 1 and (next_row, next_col) not in excluded_positions):
          return False
    ...
    def adjacent_hexes(self, row, col):
        if col % 2 == 0:
            adjacent_hexes = [(row + 1, col), (row, col - 1), (row, col + 1),
                              (row - 1, col), (row - 1, col + 1), (row - 1, col - 1)]
        else:
            adjacent_hexes = [(row + 1, col), (row + 1, col - 1),
                              (row + 1, col + 1), (row - 1, col), (row, col + 1), (row, col - 1)]
    ...
    def is_game_done(self):
        rewards = [0, 0]
        white_intellector_present = np.any(self.board[0, :, :] == 2)
        black_intellector_present = np.any(self.board[1, :, :] == 2)
        ...
        if not white_intellector_present:
            ...
            _, _, actions_white = self.get_all_actions(0)
            _, _, actions_black = self.get_all_actions(1)
    ...
    def is_empty(self, pos, turn: int) -> bool:
        return self.board[turn, pos[0], pos[1]] == 0

    def is_enemy(self, pos, turn: int) -> bool:
      return self.board[1 - turn, pos[0], pos[1]] != 0
    ...
    def is_in_range(self, pos) -> bool:
        row, col = pos
        excluded_positions = [(6, 1), (6, 3), (6, 5), (6, 7)]
        return (0 <= row < self.grid_height and 0 <= col < self.grid_width - 1
    ...
    def get_actions_for_progressor(self, pos, turn: int):
      ...
      if turn == 0:
          if col % 2 == 0:
              ...
              progressor_moves = [(row + 1, col), (row + 1, col - 1), (row + 1, col + 1)]
      else:
          if col % 2 == 0:
              progressor_moves = [(row - 1, col), (row - 1, col + 1), (row - 1, col - 1)]
          else:
    ...
    def get_actions_for_agressor(self, pos, turn: int):
      ...
      for i in range(col + 2, self.grid_width, 2):
          next_pos = (row, i)
          if not self.is_valid_move(next_pos, turn) or not self.is_empty(next_pos, turn):
              break
    ...
    def get_actions_for_liberator(self, pos, turn: int):
      ...
      for i, (r, c) in enumerate(liberator_moves):
          next_pos = (r, c)
          if not self.is_valid_move(next_pos, turn) or self.board[turn, r, c] != 0 or self.board[1 - turn, r, c] != 0 :
              continue

          possibles[i] = next_pos
          actions_mask[i] = 1

      moves = [(row + 2, col), (row - 2, col), (row + 1, col + 2),
    ...
    def choose_color(self, row, col):
        color_peru = [(1, 1), (1, 3), (1, 5), (1, 7), (1, 9),
                      (2, 2), (2, 4), (2, 6), (2, 8),
                      (4, 1), (4, 3), (4, 5), (4, 7), (4, 9),
                      (5, 2), (5, 4), (5, 6), (5, 8),
                      (7, 1), (7, 3), (7, 5), (7, 7), (7, 9)]
    ...
    def get_col_letters(self):
        return {i: chr(65 + i - 1) for i in range(1, self.grid_width)}  # A = 65 in ASCII
    ...
    def calculate_hex_position(self, dx, dy, col, original_row):
        x = dx * col
    ...
    def draw_piece_from_board(self, ax, row, col, x, y, col_letters):
      ...
      if white_figure > 0:
          ...
          ax.text(x, y, f'{row}{col_letters[col]}{color_prefix}{symbol}', ha='center', va='center', color='black', fontsize=10)
      elif black_figure > 0:
          color_prefix = 'b'
          symbol = self.FIGURE_SYMBOLS[black_figure]
          ax.text(x, y, f'{row}{col_letters[col]}{color_prefix}{symbol}', ha='center', va='center', color='black', fontsize=10)
      else:
          ax.text(x, y, f'{row}{col_letters[col]}', ha='center', va='center', color='black', fontsize=10)
    ...
    def display_plot(self, fig, mode):
        if mode == "human":
            plt.show()
    ...
    def get_all_actions(self, turn: int):
      ...
      return np.array(all_source_pos), np.array(all_possibles), np.array(all_actions_mask)
    ...
    def move_piece(self, current_pos, next_pos, turn: int):
      ...
      if self.board[1 - turn, next_row, next_col] != 0:
          self.board[1 - turn, next_row, next_col] = 0
          to_remove = None
          for key, value in self.pieces[1 - turn].items():
              if np.array_equal(value, next_pos):  # Использование np.array_equal для сравнения
                  to_remove = key
                  break
          if to_remove:
              self.pieces[1 - turn][to_remove] = None  # Установка позиции в None вместо удаления

      # Перемещение фигуры игрока на новую позицию
      self.board[turn, next_row, next_col] = self.board[turn, current_row, current_col]
      self.board[turn, current_row, current_col] = 0

      # Обновление позиции в списке фигур
      for key, value in self.pieces[turn].items():
          if np.array_equal(value, current_pos):  # Использование np.array_equal для сравнения
    ...
    def step(self, action: int):
        ...
        rewards, infos = self.move_piece(source_pos[action], possibles[action], self.turn)
        observation = self.get_state(self.turn)
        self.turn = 1 - self.turn
        self.steps += 1
        done, game_rewards = self.is_game_done()
        rewards = [sum(x) for x in zip(rewards, game_rewards)]  # Суммировать награды текущего хода и конца игры
        return observation, rewards, done, self.steps >= self.max_steps, {}